...
function createCluster(clusterId, centerX, centerY, centerZ, config) {
  const neurons = [];
  const spread  = config.neuronSpread;
  const total   = config.neuronsPerCluster;
...
export function createNetwork(config) {
  ...
  for (const center of clusterCenters) {
    allNeurons.push(...createCluster(center.id, center.x, center.y, center.z, config));
  }
