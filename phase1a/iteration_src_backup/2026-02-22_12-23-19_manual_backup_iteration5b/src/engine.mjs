...
export function step(network, inputPattern, targetPattern) {
  ...
  const accuracy    = correct / outputNeurons.length;
  // Graded reward: near-50% accuracy → near-zero signal; high/low accuracy → strong signal.
  // With rewardExponent=2, a fixed-output strategy averaging ~55% gets ≈0.01 reward
  // vs a pattern-specific 80% correct getting 0.36. Falls back to linear when exponent=1.
  const _linear = (accuracy - 0.5) * 2; // range [-1, +1]
  const rewardSignal = Math.sign(_linear) * Math.pow(Math.abs(_linear), config.rewardExponent ?? 1);
