/**
 * Experiment 010: Direction-consistent flag gating.
 * Base: experiment-004 (flag gate). Problem from experiment-008: flags saturate
 * uniformly (90%+ latched by ep 100 on ALL seeds, good or poor) because any
 * 2 consecutive non-zero traces suffice to latch with gain=0.3.
 *
 * Fix: require consecutiveConsistent >= consistencyThreshold same-sign traces
 * before flagStrength grows. A direction flip resets the streak and applies
 * flagDecayOnFlip penalty. Silence is neutral (does not reset streak or sign).
 *
 * consistencyThreshold: 5    — 5 consecutive same-sign traces to unlock growth
 * flagStrengthGain:     0.15 — slower growth (was 0.3)
 * flagDecayOnFlip:      0.5  — sharp penalty on direction flip
 * flagDecayRate:        0.9  — passive decay on silence (was 0.7)
 * flagStrengthThreshold: 0.5 — unchanged: gate still at |flagStrength| >= 0.5
 *
 * Prediction: latched % at ep 500 should be much lower than exp-008's 90%+,
 * and the gate should now be genuinely selective.
 *
 * Same 10-seed harness as multi-seed.mjs. Reports ep-500 latch % per seed
 * for the Full model condition.
 */
