/**
 * Experiment 013: 012b parameters + propagationCycles: 3.
 * Base: experiment-012b (exp-004 + weightDecay: 0.0025).
 * One addition: propagationCycles: 3 — forward pass runs 3× per training step,
 * letting signals propagate further through the recurrent graph before learning.
 *
 * Full config:
 ...
 *   propagationCycles:    3      ← the one new addition
 *   reward:               linear (no squaring, no annealing)
 *
 * No direction-consistent flags, no frustration flip, no reward shaping.
 *
 * Experiment-010b showed propagationCycles=3 hurt when combined with
 * direction-consistent flags (46.5%). The question here: does 012b's
 * stronger weight foundation allow the network to benefit from multi-cycle
 * propagation instead of being destabilised by it?
 *
 * Same 10-seed harness, 4 conditions, 1000 episodes, frozen eval.
 * Diagnostic: mean |weight| per seed at ep 1000, Full model.
 */
...
// ─── Main ─────────────────────────────────────────────────────────────────
console.log('mcfeedback — Experiment 013: 012b + propagationCycles=3\n');
console.log('Seeds:', SEEDS.join(', '));
console.log('weightDecay: 0.0025  propagationCycles: 3\n');
...
// ─── Comparison row vs 012b ───────────────────────────────────────────────
console.log('\n─── Comparison: 012b vs 013 (Full model) ────────────────────────────────');
const E12B_REF = { mean: 55.0, std: 0.0, seeds55: 10, meanW: 0.7898 };
const fm013Mean = mean(FM.map(r => r.mean)) * 100;
const fm013Std  = std(FM.map(r => r.mean)) * 100;
const fm013Good = FM.filter(r => r.mean >= 0.55).length;
const fm013W    = mean(FM.map(r => r.meanAbsW));
console.log(`${''.padEnd(20)} ${'012b'.padEnd(14)} ${'013 (012b+cycles=3)'.padEnd(20)}`);
console.log('─'.repeat(54));
console.log(`${'Mean accuracy'.padEnd(20)} ${(E12B_REF.mean.toFixed(1) + '%').padEnd(14)} ${(fm013Mean.toFixed(1) + '%').padEnd(20)}`);
console.log(`${'Std'.padEnd(20)} ${(E12B_REF.std.toFixed(1) + '%').padEnd(14)} ${(fm013Std.toFixed(1) + '%').padEnd(20)}`);
console.log(`${'Seeds ≥ 55%'.padEnd(20)} ${(E12B_REF.seeds55 + '/10').padEnd(14)} ${(fm013Good + '/10').padEnd(20)}`);
console.log(`${'Mean |weight|'.padEnd(20)} ${E12B_REF.meanW.toFixed(4).padEnd(14)} ${fm013W.toFixed(4).padEnd(20)}`);
