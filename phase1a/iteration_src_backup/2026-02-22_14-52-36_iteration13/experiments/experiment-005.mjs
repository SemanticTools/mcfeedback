/**
 * Experiment 005: Graded (non-linear) reward signal + flag strengthen/decay.
 *
 * Problem: fixed-output strategies score ~55% average, receiving mild positive
 * reward every step — making them a local optimum. A squared reward squashes
 * near-50% accuracy to near-zero signal, so only genuinely discriminating
 * outputs get strong feedback.
 *
 * Change from baseline:
 *   rewardSignal = sign(acc-0.5) * |linear|^rewardExponent  (rewardExponent=2.0)
 *   flag strengthen/decay carried over from experiment-004
