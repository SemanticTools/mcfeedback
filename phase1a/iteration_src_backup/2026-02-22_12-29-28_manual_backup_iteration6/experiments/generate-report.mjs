...
function runCondition(label, configOverrides) {
  ...
  const perPattern = PATTERNS.map(pattern => {
    const r = evaluate(network, pattern.input, pattern.target);
  ...
  return { label, config: { ...config }, history, finalAccuracy, perPattern };
}
...
<h2>Final Accuracy — Frozen Weights Evaluation</h2>
<p style="font-size:0.82rem;color:#666;margin-bottom:12px;">
  Weights were frozen after training. Each pattern was presented once with a clean
  forward pass — no learning, no weight updates. This is the real test.
</p>
${(() => {
  const rows = finalTable.map(r => {
    const cells = r.perPattern.map(p => {
      const pct    = (p.accuracy * 100).toFixed(0) + '%';
      const cls    = p.accuracy >= 0.8 ? 'good' : p.accuracy < 0.6 ? 'bad' : '';
      const outStr = '[' + p.output.join(',') + ']';
      const tgtStr = '[' + p.target.join(',') + ']';
      return '<td class="' + cls + '" title="out:' + outStr + ' target:' + tgtStr + '">' + pct + '</td>';
    }).join('');
    const meanCls = parseFloat(r.finalAccuracy) >= 80 ? 'good' : '';
    return '<tr><td>' + r.label + '</td>' + cells + '<td class="' + meanCls + '">' + r.finalAccuracy + '</td></tr>';
  }).join('\n  ');
  return '<table>\n  <tr><th>Condition</th><th>Pattern 1</th><th>Pattern 2</th><th>Pattern 3</th><th>Pattern 4</th><th>Mean</th></tr>\n  ' + rows + '\n</table>';
})()}
<p style="font-size:0.78rem;color:#444;margin-top:-8px;">Hover a cell to see actual output vs target.</p>
