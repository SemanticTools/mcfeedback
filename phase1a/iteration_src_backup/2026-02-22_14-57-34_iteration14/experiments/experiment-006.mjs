/**
 * Experiment 006: Reward annealing — linear → squared over training.
 * Base: experiment-004 (flag gate + linear reward).
 *
 * Problem with exp-005: squared reward from step 1 starved early bootstrapping.
 * Solution: start with linear reward so the network can find a direction, then
 * gradually tighten to squared reward so fixed-output attractors lose their edge.
 *
 * rewardAnnealStart: 350  — episodes 0–349: pure linear (bootstrap phase)
 * rewardAnnealEnd:   700  — episodes 350–700: smooth blend; 700+: pure squared
 * rewardExponent:    2.0  — target exponent at end of anneal
 * flag gate params carried over from experiment-004.
 */
...
const BASE_CONFIG = {
  ...
  rewardExponent:              2.0,   // target exponent at end of anneal
  rewardAnnealStart:           350,   // episode where blend begins
  rewardAnnealEnd:             700,   // episode where blend completes
