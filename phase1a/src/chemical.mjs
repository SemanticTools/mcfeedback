...
export function diffuseChemicalPerBit(network, outputNeurons, targetPattern) {
  ...
  for (let i = 0; i < outputNeurons.length; i++) {
    ...
    // Correct fire  → positive (reinforce active→active)
    // Wrong fire    → negative (weaken  active→active)
    // Wrong silence → negative (strengthen active→silent via mismatch trace)
    // Correct silence → zero   (no update — sending positive here would weaken
    //                           active→silent synapses via mismatch, destroying discrimination)
    const signal =
      actual === 1 && expected === 1 ?  config.positiveRewardStrength :
      actual === 1 && expected === 0 ?  config.negativeRewardStrength :
      actual === 0 && expected === 1 ?  config.negativeRewardStrength : 0;
