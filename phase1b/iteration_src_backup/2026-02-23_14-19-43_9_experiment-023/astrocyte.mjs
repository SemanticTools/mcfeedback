...
// Experiment 023: probability of spontaneous activation regardless of score/threshold.
// Each astrocyte, each step, independently fires with this probability.
// Allows dormant territories to accumulate enough activations to discover whether
// perturbations there are useful — if they are, the threshold drops and regular
// activation takes over. If not, the astrocyte stays at background exploration rate.
...
// useEpsilon: if true, each astrocyte that doesn't qualify by score has an EPSILON
// chance of activating anyway. Epsilon activations are counted on ast.epsilonCount.
export function selectActiveAstrocytes(astrocytes, scores, useEpsilon = false) {
  ...
  for (const c of candidates) {
    ...
    if (!byScore && useEpsilon && Math.random() < EPSILON) c.byEpsilon = true;
