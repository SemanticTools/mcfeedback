...
export function train(network) {
  ...
  for (let episode = 0; episode < EPISODES; episode++) {
    ...
    for (let step = 0; step < STEPS_PER_EPISODE; step++) {
      ...
      // Baseline evaluation
      const baseOutput = propagate(network, input);
      const baseReward = computeReward(baseOutput, target);
      ...
      const newOutput = propagate(network, input);
      const newReward = computeReward(newOutput, target);

      if (newReward > baseReward) {
        accepted++;
        rewardSum += newReward;
      } else {
        revertWeights(saved);
