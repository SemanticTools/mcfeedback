// Training loop for Experiment 022.
// Dispatches to three conditions: 'astrocyte-traffic', 'astrocyte-firing', 'control'.
...
export function train(network, condition) {
  ...
  if (condition === 'astrocyte-firing')  return trainAstrocyte(network, 'firing');
  if (condition === 'astrocyte-traffic') return trainAstrocyte(network, 'traffic');
  // Legacy alias kept for backward compatibility
  if (condition === 'astrocyte')         return trainAstrocyte(network, 'firing');
...
// ─── Astrocyte condition ──────────────────────────────────────────────────────
// scoringMode: 'firing' (exp021 baseline) | 'traffic' (exp022 new mechanism)

function trainAstrocyte(network, scoringMode) {
  ...
  const label = scoringMode === 'traffic' ? 'traffic' : 'firing ';
